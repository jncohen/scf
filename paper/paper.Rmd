...
A detailed methodological description and validation against official Federal Reserve statistics is provided by @Cohen2026.
...
The `scf` package addresses this gap by formalizing SCF-specific best practices within a standardized functional interface. It provides a unified data object that encodes the replicate-weighted design across implicates, together with functions that automate estimation and pooling. The package offers a consistent grammar and output format that substantially reduces custom code requirements for common analytical tasks. It reproduces official SCF statistics and reduces typical workflows from dozens of lines of custom code to a small number of standardized function calls [see @Cohen2026].

# State of the field

No dedicated R package for SCF analysis existed on CRAN prior to `scf`. Existing approaches rely on general-purpose tools (`survey`, `mitools`) or reference documentation (`asdfree`) rather than a purpose-built functional interface. Contributing SCF-specific logic to `survey` would be inappropriate for a general-purpose package; extending `asdfree` would not yield the modular, testable, CRAN-distributed infrastructure necessary for reproducible workflows across projects and users. The `scf` package fills this gap.

# Research impact statement

The package has been validated against published Federal Reserve statistics; @Cohen2026 documents this validation in detail. Package vignettes include replications of results from two published empirical papers, demonstrating that `scf` produces estimates consistent with prior peer-reviewed findings. The package is distributed through CRAN and targets a research community for whom the SCF serves as a primary data source.
...
Rather than implementing survey-statistical computations from scratch, `scf` builds on the `survey` package [@Lumley2026], inheriting its well-established validation record and comprehensive estimator coverage. Pooling uses Rubin's combining rules [@Rubin1987] for model-based quantities and alternative strategies for percentiles and proportions per Federal Reserve documentation; these distinctions are handled automatically.
